...
def render_tab(calc_payment_func, prefix, is_lease=False):
    ...
    with st.popover("Enter Finance Details", use_container_width=True):
        ...
        drivers_license = c4.text_input(label="Driver's License", key=f"{prefix}_drivers_license")
        county = c4.text_input(label="County", key=f"{prefix}_county")
        platenum = c4.text_input(label="License Plate Number", key=f"{prefix}_platenum")
        plate_exp = c4.text_input(label="Plate Expiration", key=f"{prefix}_plate_exp")
        lienholder_name = c8.text_input(label="Lienholder Name", key=f"{prefix}_lienholder_name")
        lienholder_address = c8.text_input(label="Lienholder Address", key=f"{prefix}_lienholder_address")
        lienholder_city = c8.text_input(label="Lienholder City", key=f"{prefix}_lienholder_city")
        lienholder_state = c8.text_input(label="Lienholder State", key=f"{prefix}_lienholder_state")
        lienholder_zip = c8.text_input(label="Lienholder Zip Code", key=f"{prefix}_lienholder_zip")
