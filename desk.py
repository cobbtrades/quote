...
def render_tab(calc_payment_func, prefix, is_lease=False):
    ...
    with fc:
        ...
        zipcode = text_input_with_label(ft2, st2, "Zip", key=f"{prefix}_zip", max_chars=5)
        email_address = text_input_with_label(fc2, sc2, "Email", key=f"{prefix}_emailaddress")
    ...
    with st.popover("Enter Finance Details", use_container_width=True):
        ...
        bodystyle = text_input_with_label(c1, c2, "Body Style", key=f"{prefix}_bodystyle")
        fuel_type = text_input_with_label(c1, c2, "Fuel Type", key=f"{prefix}_fuel_type")
        drivers_license = text_input_with_label(c3, c4, "Driver's License", key=f"{prefix}_drivers_license")
        ...
        lienholder_city = text_input_with_label(c5, c6, "Lienholder City", key=f"{prefix}_lienholder_city", value=lien_city)
        lienholder_state = text_input_with_label(c5, c6, "Lienholder State", key=f"{prefix}_lienholder_state", value=lien_state, max_chars=2)
