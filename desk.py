...
def render_tab(calc_payment_func, prefix, is_lease=False):
    ...
    with st.popover("Enter Finance Details", use_container_width=True):
        ...
        plate_exp = c3.text_input(label="Plate Expiration", key=f"{prefix}_plate_exp")
        lienholder_name = c4.text_input(label="Lienholder Name", key=f"{prefix}_lienholder_name")
        lienholder_address = c4.text_input(label="Lienholder Address", key=f"{prefix}_lienholder_address")
        lienholder_city = c4.text_input(label="Lienholder City", key=f"{prefix}_lienholder_city")
        lienholder_state = c4.text_input(label="Lienholder State", key=f"{prefix}_lienholder_state")
        lienholder_zip = c4.text_input(label="Lienholder Zip Code", key=f"{prefix}_lienholder_zip")
        ins_company = c5.text_input(label="Insurance Company", key=f"{prefix}_ins_company")
        policy = c5.text_input(label="Policy #", key=f"{prefix}_policy")
