...
def render_tab(calc_payment_func, prefix, is_lease=False):
    ...
    with st.popover("Enter Finance Details", use_container_width=True):
        ...
        lienholder_name = c6.text_input(label="Lienholder Name", key=f"{prefix}_lienholder_name")
        lienholder_address = c6.text_input(label="Lienholder Address", key=f"{prefix}_lienholder_address")
        lienholder_city = c6.text_input(label="Lienholder City", key=f"{prefix}_lienholder_city")
        lienholder_state = c6.text_input(label="Lienholder State", key=f"{prefix}_lienholder_state")
        lienholder_zip = c6.text_input(label="Lienholder Zip Code", key=f"{prefix}_lienholder_zip")
