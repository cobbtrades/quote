...
with st.form(key='deal_form'):
    ...
    st.write("### Down Payment Options")
    down_payments = []
    default_down_payments = [1000.0, 2000.0, 3000.0]
    down_payment_columns = st.columns(len(default_down_payments))
    for i, col in enumerate(down_payment_columns):
        down_payments.append(col.number_input(f"Down Payment {i+1}", min_value=0.0, value=default_down_payments[i], format="%.2f", key=f'down_payment_{i+1}'))

    st.write("### Loan Terms and Rates")
    terms = []
    rates = {}
    for i in range(1, 4):
        with st.columns(2) as term_rate_cols:
            term = term_rate_cols[0].number_input(f"Loan Term {i} (months)", min_value=1, value=[60, 66, 72][i-1], key=f'term_{i}')
            rate = term_rate_cols[1].number_input(f"Rate for Term {i} (%)", min_value=0.0, max_value=100.0, value=14.0, format="%.2f", key=f'rate_{i}')
            terms.append(term)
            rates[term] = rate
